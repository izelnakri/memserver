...
function getDefaultRouteHandler(verb, path) {
  ...
  const ResourceModel = targetNamespace$1.MemServer.Models[string.classify(resourceName)];

  if (!ResourceModel) {
    throw new Error(chalk.red(`[MemServer] ${verb} ${path} route handler cannot be generated automatically: ${string.classify(resourceName)} is not a valid MemServer.Model, please check that your route name matches the model reference or create a custom handler function`));
  } else if (verb === 'GET') {
