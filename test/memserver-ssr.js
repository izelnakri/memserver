...
import FastBootExpressMiddleware from 'mber/lib/utils/fastboot-express-middleware';
...
test.beforeEach(async () => {
  ...
  await Promise.all([
    fs.mkdirp(`${CWD}/memserver/models`),
    fs.mkdirp(`${CWD}/memserver/fixtures`)
  ]);
  await Promise.all([
...
test.serial('MemServer with JSDOM could be used with ember fastboot for server side rendering', async (t) => {
  const FASTBOOT_DIST_PATH = `${process.cwd()}/test-helpers/fastboot-dist`;
