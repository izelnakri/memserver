...
test.serial('$Model.insert() will insert an empty model and auto-generate primaryKeys', async (t) => {
  t.plan(7);
...
test.serial('$Model.insert(attributes) will insert a model with overriden attributes', (t) => {
  t.plan(14);
...
test('$Model.insert(attributes) will throw if overriden primaryKey already exists', (t) => {
  t.plan(4);
  ...
  const error = t.throws(() => Photo.insert({ id: 1 }), Error);

  t.true(/\[MemServer\] Photo id 1 already exists in the database! Photo.insert\(\{ id: 1 \}\) fails/.test(error.message));

  const secondError = t.throws(() => PhotoComment.insert({ uuid: 'd351963d-e725-4092-a37c-1ca1823b57d3' }), Error);

  t.true(/\[MemServer\] PhotoComment uuid d351963d-e725-4092-a37c-1ca1823b57d3 already exists in the database! PhotoComment.insert\(\{ uuid: 'd351963d-e725-4092-a37c-1ca1823b57d3' \}\) fails/.test(secondError.message));
});

test('$Model.insert(attributes) will throw if overriden primaryKey is wrong type', (t) => {
  t.plan(4);
  ...
  const error = t.throws(() => Photo.insert({ id: '99' }), Error);

  t.true(/\[MemServer\] Photo model primaryKey type is 'id'. Instead you've tried to enter id: 99 with string type/.test(error.message));

  const secondError = t.throws(() => PhotoComment.insert({ uuid: 1 }), Error);

  t.true(/\[MemServer\] PhotoComment model primaryKey type is 'uuid'. Instead you've tried to enter uuid: 1 with number type/.test(secondError.message));
});

test('$Model.insert(attributes) can add new values to $Model.attributes when new attributes are discovered', (t) => {
  const MemServer = require('../../lib/index.js');
  const { Photo, PhotoComment } = MemServer.Models;
