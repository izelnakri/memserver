...
test.serial('POST /resources work with shortcut', async (t) => {
  ...
  await window.$.ajax({
    ...
    t.deepEqual(Photo.find(4), {
      id: 4, name: 'Izel Nakri', is_public: true, href: undefined, user_id: undefined
    });
  });
});
...
test.serial('throws an helpful error message when shortcuts model is not found', async (t) => {
  ...
  t.true(/\[MemServer\] GET \/houses route handler cannot be generated automatically: House is not a valid MemServer\.Model, please check that your route name matches the model reference or create a custom handler function/.test(error.message));
});
...
test.serial('POST /resources works correctly with undefined handler response', async (t) => {
  ...
  await window.$.ajax({
    ...
    t.deepEqual(jqXHR.responseJSON, { error: '[MemServer] POST /photos route handler did not return anything to respond to the request!' });
    t.is(Photo.count(), 3);
  });
});
...
test.serial('GET /resources works correctly with undefined handler response', async (t) => {
  ...
  await window.$.ajax({
    ...
    t.is(jqXHR.status, 500);
    t.deepEqual(jqXHR.responseJSON, { error: '[MemServer] GET /photos route handler did not return anything to respond to the request!' });
  });
});
...
test.serial('GET /resources/:id works correctly with undefined handler response', async (t) => {
  ...
  await window.$.ajax({
    ...
    t.is(jqXHR.status, 500);
    t.deepEqual(jqXHR.responseJSON, { error: '[MemServer] GET /photos/1 route handler did not return anything to respond to the request!' });
  });
});
...
test.serial('PUT /resources/:id works correctly with undefined handler response', async (t) => {
  ...
  await window.$.ajax({
    ...
    t.is(jqXHR.status, 500);
    t.deepEqual(jqXHR.responseJSON, { error: '[MemServer] PUT /photos/1 route handler did not return anything to respond to the request!' });
  });
});
