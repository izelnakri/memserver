...
test.serial('[MemServer.Server] POST /resources work with custom headers, queryParams and responses', async (t) => {
  t.plan(8);
...
test.serial('[MemServer.Server] GET /resources works with custom headers, queryParams and responses', async (t) => {
  t.plan(6);
...
test.serial('[MemServer.Server] GET /resources/:id works with custom headers, queryParams and responses', async (t) => {
  t.plan(4);
...
test.serial('[MemServer.Server] PUT /resources/:id works with custom headers, queryParams and responses', async (t) => {
  t.plan(4);
...
test.serial('[MemServer.Server] DELETE /resources/:id works with custom headers, queryParams and responses', async (t) => {
  t.plan(3);
...
test.serial('[MemServer.Server Edge cases] Server works for coalasceFindRequests routes', async (t) => {
  t.plan(6);
...
test.serial('[MemServer.Server Edge cases] Server converts empty strings to null during a request and formats query params', async (t) => {
  t.plan(4);
...
test.serial('[MemServer.Server Edge cases] Server casts uuids correct as queryParams', async (t) => {
  t.plan(2);
...
test.serial('[MemServer.Server Edga cases] Server casts ethereum adresses correctly as string request.params', async (t) => {
  t.plan(2);
...
test.serial('[MemServer.Server Edge cases] Server casts ethereum addresses correctly as string in request.queryParams', async (t) => {
  t.plan(2);
...
async function writeEdgeCasesServerFile() {
  await Promise.all([
    fs.writeFile(`${CWD}/memserver/models/ethereum-account.js`, `
      import Model from '${CWD}/lib/model';

      export default Model({
      });
    `),
    fs.writeFile(`${CWD}/memserver/fixtures/ethereum-accounts.js`, `export default [
      {
        id: 1,
        address: '0x7be8315acfef37816c9ad4dc5e82195f2a52934c5d0c74883f9978675e26d600'
      }
